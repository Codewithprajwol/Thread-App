...
const ActionForReply = ({post,reply}) => {
    const user=useAuthStore((state)=>state.user);
    const [liked,setLiked]=useState(reply?.likes?.includes(user._id));
    const likeUnlikeComment=useReplyStore((state)=>state.likeUnlikeComment);
    ...
    const handleLikeUnlikePost=async()=>{
        ...
        await likeUnlikeComment({ id:reply._id,liked:liked,userId:user._id})
        setLiked(!liked);
    }
     useEffect(()=>{
        if(post?._id){
      ...
      <DialogContent aria-describedby={undefined}  className="sm:max-w-[425px] px-2 sm:px-4">
        ...
        <div className="px-0 flex items-end justify-center gap-4 flex-col">
          ...
          <Button type="button" onClick={handleReplySubmit} className='cursor-pointer'>{isReplying?(
              ...
              <p>{reply?.children?.length} replies</p>
          </div>
                </div>
)
};

export default ActionForReply;
